...
WIDGET = {
    'type': 'tautulli_streams',
    'name': 'Tautulli Streams',
    'integration_types': ['tautulli'],
    'default_size_x': 8,
    'default_size_y': 5,
    'icon': 'bar-chart-2',
    'config_schema': {
...
CACHE_NS = 'tautulli_streams'
CACHE_TTL = 20


def register(app):
    @app.route('/api/tautulli_streams/data', methods=['GET'], endpoint='widget_tautulli_streams_data')
    def tautulli_streams_data():
        ...
        integration, config, err, code = get_integration_for_widget(widget_id, ['tautulli'])
        if err:
        ...
        try:
            result = iwf.fetch_tautulli_streams(config)
            result['configured'] = True
            result['online'] = True
            widget_cache.set(ck, result, ttl=CACHE_TTL)
            return jsonify(result)
        except Exception as exc:
            logger.warning('tautulli_streams failed: %s', exc)
