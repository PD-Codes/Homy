...
CACHE_NS = 'adguard_status'
CACHE_TTL = 30


def register(app):
    @app.route('/api/adguard_status/data', methods=['GET'], endpoint='widget_adguard_status_data')
    def adguard_status_data():
        ...
        try:
            result = iwf.fetch_adguard_status(config)
            result['configured'] = True
            result['online'] = True
            widget_cache.set(ck, result, ttl=CACHE_TTL)
            return jsonify(result)
        except Exception as exc:
            logger.warning('adguard_status failed: %s', exc)
